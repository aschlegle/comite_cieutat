...
2. **One container**: `.etiquette` — the cream, scallop-edged "stamp" from the stories. Every section is one etiquette; pages are just a `.pile` of them (`display:flex; column; gap`). Inside an etiquette, sub-blocks use `.carte` (plain white, rounded). Nothing else.
   ...
   Ornaments are sized uniformly except bunting, which is wide and thin and gets its own width via `.decor[src$="fanions.png"]`. That selector outranks the plain `.decor` rule in the small-screen media query, so the mobile override has to repeat the attribute selector — otherwise the bunting stays large and overlaps headings.
