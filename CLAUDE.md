...
2. **One container**: `.etiquette` — the cream, scallop-edged "stamp" from the stories. Every section is one etiquette; pages are just a `.pile` of them (`display:flex; column; gap`). Inside an etiquette, sub-blocks use `.carte` (plain white, rounded). Nothing else.
   ...
   Corner ornaments live in `public/assets/decor/`, placed with `.decor` + `.decor-hg/-hd/-bg/-bd`, always `alt="" aria-hidden="true"`. Two kinds:
   - **`.png` — the committee's own Canva artwork** (bunting, clinking glasses, cutlery). Always prefer these; ask for the original asset before drawing anything. They arrive as opaque white-background RGB, so they must be keyed out first: rebuild the alpha from each pixel's distance to white and un-premultiply the ink colour (a plain white→transparent threshold leaves a halo), then `Image.quantize(colors=64, method=FASTOCTREE)` — that step alone takes them from ~45kB to ~10kB.
   - **`.svg` — reconstructions** for the motifs no original was supplied for (sparkles, mirror ball, balloons, scallop shell, starfish, party hats). They match the stories' stroke recipe: `fill:none`, `stroke:#1a2a72`, `stroke-width:2.6`, round caps, 100×100 viewBox. Any new one must match it or not exist.

   A section may carry an ornament in **all four corners** (the meal/menu sections do) — but only if its heading block is centred, and it must add `.etiquette--decore` for the extra padding. With a left-aligned heading the top-left ornament lands straight on the title.

   Ornaments are sized uniformly except bunting and clinking glasses, which get their own width via `.decor[src$="..."]` — bunting because it is wide and thin, glasses because the drawing is fine and airy and turns to mush at the common size. Bunting rises to the right, so `.decor-hd` / `.decor-bd` flip it with `transform: scaleX(-1)` to anchor it into the corner. That selector outranks the plain `.decor` rule in the small-screen media query, so the mobile override has to repeat the attribute selector — otherwise the bunting stays large and overlaps headings.
